...
float calculPromedio(float notas[5]) {
	float suma = 0;
	for (int i = 0; i < 5; i++) {
		suma += notas[i];
	}
...
void verifiApellido(char apellido[50]) {
...
int main() {
	...
	for (int i = 0; i < 3; i++) {
		...
		scanf("%s", apellidos[i]);
		verifiApellido(apellidos[i]);
		
		printf("Ingrese las notas (5) %d:\n", i + 1);
		for (int j = 0; j < 5; j++) {
			scanf("%f", &notas[i][j]);
		}
		
		promedios[i] = calculPromedio(notas[i]);
